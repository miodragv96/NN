...
for i=1:length(trainedNetwork)
    for j=1:size(trainedNetwork{i},1)
        ...
        %Sum up all positive and negative weights seperatly (Bias is ignored)
        ...
        %For case that the positive number is equal to n to the power of 2
...
%For case that the number is equal to n to the power of 2
%increase the tempMax by one
%Because integer range is -2^n:2^n-1
